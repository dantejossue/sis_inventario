...
$(function () {
  const modal = $('#modalEditarMarca');
  const form = $('#formEditarMarca');
  const btn = $('#btnActualizarMarca');
  const spinner = btn.find('.spinner-border');

  $('#miTablaMarcas').on('click', '.btn-editar-marca', function () {
    const b = $(this);

    limpiarErrores();

    $('#edit-nombre-marca').val(b.data('nombre'));
  ...
  form.on('submit', function (e) {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo actualizar la marca.' });
      }
    });
  });
  ...
  function encontrarFila(id) {
    let found = null;
    window.tablaMarcas.rows().every(function () {
      if (this.data().id_marca == id) found = this;
    });
    return found;
  }
