...
$(function () {
  const modal   = $('#modalEditarCategoria');
  const form    = $('#formEditarCategoria');
  const btn     = $('#btnActualizarCategoria');
  const spinner = btn.find('.spinner-border');

  $('#miTablaCategorias').on('click', '.btn-editar', function () {
    const b = $(this);
  ...
  function encontrarFila(id) {
    let found = null;
    window.tablaCategorias.rows().every(function () {
      if (this.data().id_categoria == id) found = this;
    });
    return found;
  }
