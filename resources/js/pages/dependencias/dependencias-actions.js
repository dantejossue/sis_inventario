...
$(function () {
  ...
  $('#miTablaDependencias').on('click', '.btn-toggle-estado', function () {
    const btn = $(this);
    ...
    Swal.fire({
      title: '¿Estás seguro?',
      text: `Vas a ${accion} la dependencia.`,
      icon: 'warning',
      ...
      $.ajax({
        url: btn.data('url'), type: 'POST',

        success(res) {
          if (res.success) {
            const row = window.tablaDependencias.row(btn.closest('tr'));
            const d   = row.data();
